...
class MetricsTests(unittest.TestCase):
    ...
    def test_list_of_metrics(self):
        ...
        test_list_with_abbr = he.list_of_metrics(
            he.metric_abbr, self.sim, self.obs, abbr=True
        )
        ...
        self.assertTrue(
            "One or both of the ndarrays are not 1 dimensional."
...
class EnsMetricsTests(unittest.TestCase):
    def setUp(self):
        self.ensemble_array = np.load(r"Files_for_tests/ensemble_array.npz")[
            "arr_0.npy"
        ]
        self.observed_array = np.load(r"Files_for_tests/observed_array.npz")[
            "arr_0.npy"
        ]
        ...
        self.ensemble_array_bad_data = np.load(
            r"Files_for_tests/ensemble_array_bad_data.npz"
        )["arr_0.npy"]
        self.observed_array_bad_data = np.load(
            r"Files_for_tests/observed_array_bad_data.npz"
        )["arr_0.npy"]
    ...
    def test_ens_mae(self):
        ...
        expected_value_bad_data = em.ens_mae(
            obs=self.observed_array[8:], fcst_ens=self.ensemble_array[8:, :]
        )
    ...
    def test_ens_mse(self):
        ...
        expected_value_bad_data = em.ens_mse(
            obs=self.observed_array[8:], fcst_ens=self.ensemble_array[8:, :]
        )
    ...
    def test_ens_rmse(self):
        ...
        expected_value_bad_data = em.ens_rmse(
            obs=self.observed_array[8:], fcst_ens=self.ensemble_array[8:, :]
        )
    ...
    def test_ens_crps(self):
        ...
        crps_numba = em.ens_crps(obs=self.observed_array, fcst_ens=self.ensemble_array)[
            "crps"
        ]
        crps_python = em.ens_crps(
            obs=self.observed_array, fcst_ens=self.ensemble_array, llvm=False
        )["crps"]

        self.assertTrue(np.all(np.isclose(expected_crps, crps_numba)))
        self.assertTrue(np.all(np.isclose(expected_crps, crps_python)))

        crps_mean_numba = em.ens_crps(
            obs=self.observed_array, fcst_ens=self.ensemble_array
        )["crpsMean"]
        crps_mean_python = em.ens_crps(
            obs=self.observed_array, fcst_ens=self.ensemble_array, llvm=False
        )["crpsMean"]
        ...
        self.assertTrue(
            np.all(
                np.isclose(expected_value_bad_data["crps"], test_value_bad_data["crps"])
            )
        )
        self.assertEqual(
            expected_value_bad_data["crpsMean"], test_value_bad_data["crpsMean"]
        )

    def test_ens_pearson_r(self):
        expected_pearson_r = -0.13236871294739733
        test_pearson_r = em.ens_pearson_r(
            obs=self.observed_array, fcst_ens=self.ensemble_array
        )
        ...
        self.assertTrue(
            np.isclose(expected_pearson_r_bad_data, test_pearson_r_bad_data)
        )
    ...
    def test_crps_hersbach(self):
        ...
        crps_dictionary_test = em.crps_hersbach(
            obs=self.observed_array, fcst_ens=self.ensemble_array
        )

        self.assertTrue(np.all(np.isclose(expected_crps, crps_dictionary_test["crps"])))
        self.assertTrue(
            np.all(np.isclose(expected_mean_crps, crps_dictionary_test["crpsMean1"]))
        )
        self.assertTrue(
            np.all(np.isclose(expected_mean_crps, crps_dictionary_test["crpsMean2"]))
        )
    ...
    def test_crps_kernel(self):
        ...
        crps_dictionary_test = em.crps_kernel(
            obs=self.observed_array, fcst_ens=self.ensemble_array
        )

        self.assertTrue(np.all(np.isclose(expected_crps, crps_dictionary_test["crps"])))
        self.assertTrue(
            np.all(np.isclose(expected_mean_crps, crps_dictionary_test["crpsMean"]))
        )
    ...
    def test_ens_brier(self):
        ...
        brier_scores_test_bin = em.ens_brier(
            fcst_ens_bin=self.ens_bin, obs_bin=self.obs_bin
        )
        ...
        expected_scores_diff_thresh = np.load(
        ...
        brier_scores_test_diff_thresh = em.ens_brier(
            fcst_ens=self.ensemble_array,
            obs=self.observed_array,
            obs_threshold=180,
            ens_threshold=170,
        )

        np.testing.assert_allclose(
            expected_scores_diff_thresh, brier_scores_test_diff_thresh
        )
        self.assertAlmostEqual(
            expected_scores_diff_thresh_mean, brier_scores_test_diff_thresh.mean()
        )
    ...
    def test_auroc(self):
        ...
        auroc_test = em.auroc(
            fcst_ens=self.ensemble_array, obs=self.observed_array, threshold=180
        )
    ...
    def test_skill_score(self):
        ...
        skill_score_test = em.skill_score(
            np.array([0.1, 0.2, 0.15]), np.array([0.3, 0.4, 0.35]), 0
        )
        ...
        nan_skill_score = em.skill_score(
            np.array([0.1, 0.2, 0.15]), np.array([0.0, 0.0, 0.0]), 0
        )
...
class AnalysisTests(unittest.TestCase):
    ...
    def test_make_table(self):
        ...
        # Using the Function
        table = ha.make_table(
            self.merged_df, my_metrics, seasonal, remove_neg=True, remove_zero=True
        )
    ...
    def test_lag_analysis(self):
        ...
        time_lag_df, summary_df = ha.time_lag(
            self.merged_df, metrics=["ME", "r2", "RMSE", "KGE (2012)", "NSE"]
        )

        time_lag_df_original = pd.read_csv(
            os.path.join(os.getcwd(), "Comparison_Files", "time_lag_df.csv"),
            index_col=0,
        )

        summary_df_original = pd.read_csv(
            os.path.join(os.getcwd(), "Comparison_Files", "summary_df.csv"), index_col=0
        )
...
class VisualTests(unittest.TestCase):
    ...
    def test_hist_df(self):
        ...
        img_original = mpimg.imread(
            os.path.join(os.getcwd(), "baseline_images", "plot_tests", "hist1.png")
        )
    ...
    def test_hist_znorm(self):
        ...
        # Reading original image
        img_original = mpimg.imread(
            os.path.join(os.getcwd(), "baseline_images", "plot_tests", "hist_znorm.png")
        )
    ...
    def test_scatter(self):
        ...
        # Reading original image
        img_original = mpimg.imread(
            os.path.join(os.getcwd(), "baseline_images", "plot_tests", "scatter.png")
        )
    ...
    def test_scatterlog(self):
        ...
        # Reading original image
        img_original = mpimg.imread(
            os.path.join(os.getcwd(), "baseline_images", "plot_tests", "scatterlog.png")
        )
    ...
    def test_qq_plot(self):
        ...
        # Reading original image
        img_original = mpimg.imread(
            os.path.join(os.getcwd(), "baseline_images", "plot_tests", "qqplot.png")
        )
    ...
    def test_qq_plot2(self):
        ...
        # Reading original image
        img_original = mpimg.imread(
            os.path.join(os.getcwd(), "baseline_images", "plot_tests", "qqplot2.png")
        )
...
class DataTests(unittest.TestCase):
    ...
    def test_julian_to_gregorian(self):
        ...
        test_df = pd.DataFrame(
            data=np.random.rand(10, 2),  # Random data in the columns
        ...
        hd.julian_to_gregorian(
            test_df, inplace=True, frequency="H"
        )  # Change index values inplace
    ...
    def test_monthly_std_error(self):
        original_df = pd.read_csv(
            r"Comparison_Files/monthly_std_error.csv", index_col=0
        )
        original_df.index = np.array(
            ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"],
            dtype=np.object,
